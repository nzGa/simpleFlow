...
Future<void> _createAndPutDebugData() async {
  ...
  Account accountAlpha =
      Account(
        ...
        ..updateBalanceAndSave(384500, title: "Balance Nov 2023")
...
Future<void> addDummyData() async {
  Account account = await ObjectBox().box<Account>().getAllAsync().then(
    (value) => value[Random().nextInt(value.length)],
  );

  Category category = await ObjectBox().box<Category>().getAllAsync().then(
    (value) => value[Random().nextInt(value.length)],
  );
  ...
  account.createAndSaveTransaction(
    amount:
        (Random().nextDouble() * (randomVal.max - randomVal.min) +
            randomVal.min) *
        randomVal.multiplier,
    category: category,
    title:
        "R_${["turkey", "rock", "end", "hydrant", "stove", "act", "laugh", "team", "zebra", "rose", "nose", "friends", "channel", "tree", "root", "increase", "trousers", "school", "tin", "egg", "fact", "cherry", "laborer", "hall", "chance", "addition", "request", "wrist", "pipe", "war", "size", "rice", "stem", "toad", "twig", "event", "silver", "rings", "grass", "development", "hill", "pickle", "carpenter", "view", "cave", "test", "hose", "education", "worm", "trouble", "cap", "quilt", "lumber", "baby", "hour", "sack", "quiet", "chin", "thread", "servant", "animal", "song", "babies", "walk", "woman", "gold", "soap", "lake", "spot", "example", "rest", "glass", "minute", "hand", "thrill", "reason", "pocket", "thumb", "road", "ticket", "lock", "magic", "collar", "middle", "grandmother", "nerve", "passenger", "basketball", "bridge", "veil", "bikes", "friend", "hope", "coal", "duck", "edge", "flame", "page", "grape", "smoke"][Random().nextInt(100)]}",
  );
}
