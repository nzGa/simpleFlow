...
class _TransactionsSelectionScopeState
  ...
  Future<void> _bulkChangeCategory() async {
    ...
    final Set<TransactionType> types = selected.map((t) => t.type).toSet();
    final TransactionType? inferredType = types.length == 1
        ? types.single
        : null;
    ...
    final Optional<Category>? result =
        await showModalBottomSheet<Optional<Category>>(
          context: context,
          builder: (context) =>
              SelectCategorySheet(transactionType: inferredType),
          isScrollControlled: true,
        );
